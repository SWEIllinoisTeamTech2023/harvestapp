...
def getFuelPrice():
    ...
    print("Fuel Price: ",  resp['response']['data'][0]['value'], resp['response']['data'][0]['units'])
...
@app.route('/storeOutputs', methods=['GET'])
def storeOutputs():
    # Testing purposes, this data should be sent over from model
    # id should represent foreign key from inputs table
    data = {"fan_speed":30, "rotor_speed":12, "concave_clearance":200, "speed":30, "chaffer_clearance": 20, "sieve_clearance": 10, "optimized_cost_of_harvest": 100,"id":123}
    rdsData.execute_statement(
        resourceArn = CLUSTER_ARN,
        secretArn = SECRET_ARN,
        database = 'harvest',
